<template>
  <el-form :model="model" label-width="120px" class="edit-form" :rules="rules" ref="form"
    ...
    <el-form-item label="标题" prop="title" required>
      <el-input v-model.trim="model.title" style="width: 75%" :maxlength="30" placeholder="请输入标题"></el-input>
    </el-form-item>
    ...
    <el-form-item label="排序值" prop="sort">
      ...
      <div style="color: #f56c6c;">注意：排序值越大显示在越前面</div>
    </el-form-item>
    ...
    <el-form-item label="跳转地址" prop="link_url">
      <el-input v-model.trim="model.link_url" style="width: 75%" :maxlength="300" placeholder="请输入跳转地址"></el-input>
    </el-form-item>
  </el-form>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      init () {
        if (this.model.id > 0) {
          this.fileList = [{ url: this.model.full_image_url }]
        } else {
          this.fileList = []
        }
      },
      ...
      submit (callback) {
        this.$refs.form.validate(valid => {
          if (valid) {
            ...
            if (!this.model.id) {
              this.$service.banner.add(this.model).then(() => {
                this.$message.success('添加成功')
                callback.call()
              })
            } else {
              this.$service.banner.update(this.model).then(() => {
